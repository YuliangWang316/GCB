...
Glutamine_1<-read.table("I:/MAC/BOAO/GO_BP_Glutamine.txt",sep = "\t",header = TRUE)
...
plasma<-read.table("I:/MAC/BOAO/plasma_vs_naive_IRF4_new.tsv",sep = "\t",header = TRUE)
...
KEGGcellcycle<-read.table("I:/MAC/BOAO/Cellcycle.txt",sep = "\t",header = TRUE)
...
MTOR_1[,"Mtor_1"]<-MTOR_1[,"Mtor_1"]+noise
MTOR_1$ident<-factor(MTOR_1$ident,levels = names(x=rev(x=sort(x=tapply(X=MTOR_1[,"Mtor_1"],INDEX = MTOR_1$ident,FUN = mean),decreasing = FALSE))))
write.table(MTOR_1,file = "I:/MAC/BOAO/untitled folder/MTOR1.txt",sep = "\t")
...
GLUTAMINE_1[,"Glutamine_1"]<-GLUTAMINE_1[,"Glutamine_1"]+noise
GLUTAMINE_1$ident<-factor(GLUTAMINE_1$ident,levels = names(x=rev(x=sort(x=tapply(X=GLUTAMINE_1[,"Glutamine_1"],INDEX = GLUTAMINE_1$ident,FUN = mean),decreasing = FALSE))))
write.table(GLUTAMINE_1,file = "I:/MAC/BOAO/untitled folder/GLUTAMINE1.txt",sep = "\t")
...
MTOR_2[,"Mtor_2"]<-MTOR_2[,"Mtor_2"]+noise
MTOR_2$ident<-factor(MTOR_2$ident,levels = names(x=rev(x=sort(x=tapply(X=MTOR_2[,"Mtor_2"],INDEX = MTOR_2$ident,FUN = mean),decreasing = FALSE))))
write.table(MTOR_2,file = "I:/MAC/BOAO/untitled folder/MTOR2.txt",sep = "\t")
...
DZ<-read.table("I:/MAC/BOAO/DZ_NI_new.txt",sep = "\t",header = TRUE)
LZ<-read.table("I:/MAC/BOAO/LZ_NI_new.txt",sep = "\t",header = TRUE)
...
VlnPlot(pbmc.copy,features = "Irf4",pt.size = 0)
VlnPlot(pbmc.copy,features = "Kdm6b",pt.size = 0)
VlnPlot(pbmc.copy,features = "Ly75",pt.size = 0)
VlnPlot(pbmc.copy,features = "Mtor_1",pt.size = 0)+geom_boxplot(width=.1,col="black",fill="white")
VlnPlot(pbmc.copy,features = "Mtor_2",pt.size = 0)+geom_boxplot(width=.1,col="black",fill="white")
VlnPlot(pbmc.copy,features = "Mtor_3",pt.size = 0)+geom_boxplot(width=.1,col="black",fill="white")
VlnPlot(pbmc.copy,features = "Kdm6a",pt.size = 0)
VlnPlot(pbmc.copy,features = "Ezh2",pt.size = 0)

VlnPlot(pbmc.copy,features = "Glutamine_1",pt.size = 0)+geom_boxplot(width=.1,col="black",fill="white")
...
VlnPlot(Myc1Myc2,features = "plasma",pt.size = 0,sort = TRUE)+geom_boxplot(width=.1,col="black",fill="white")
VlnPlot(Myc1Myc2,features = "KEGGcellcycle",pt.size = 0,sort = TRUE)+geom_boxplot(width=.1,col="black",fill="white")
...
write.table(myc1,file = "I:/MAC/BOAO/myc1.txt",sep = "\t")
write.table(myc2,file = "I:/MAC/BOAO/myc2.txt",sep = "\t")
write.table(myc1s,file = "I:/MAC/BOAO/myc1s.txt",sep = "\t")
write.table(myc2s,file = "I:/MAC/BOAO/myc2s.txt",sep = "\t")
pbmc.markers<-FindAllMarkers(pbmc,only.pos = TRUE,logfc.threshold = 0,min.pct = 0)
write.table(pbmc.markers,file = "I:/MAC/BOAO/untitled folder/pbmc.markers.txt",sep = "\t")
pbmc.markers %>%
  group_by(cluster) %>%
  top_n(n = 10, wt = avg_logFC) -> top10
DoHeatmap(pbmc, features = top10$gene) + NoLegend()
pbmc.avg<-AverageExpression(pbmc)
write.table(pbmc.avg$RNA,file = "I:/MAC/BOAO/untitled folder/pbmc.avg.txt",sep = "\t")
WXMlist<-read.table("I:/MAC/BOAO/WXMlist.txt",header = TRUE,sep = "\t")
...
write.table(myc1_myc2,file = "I:/MAC/BOAO/untitled folder/my1_vs_myc2.txt",sep = "\t")
