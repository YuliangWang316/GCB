  ...
  for (p in c(50,100,200,300,400,500,600,700,800,900,1000)) {
    for (q in c(20,30,40,50,60,70,80,90,100)) {
      for (r in seq(0.8,10,by=0.1)) {
        ...
        pbmc3.markers <- FindAllMarkers(pbmc3, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
        pbmc4.markers <- FindAllMarkers(pbmc4, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
        pbmc5.markers <- FindAllMarkers(pbmc5, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
        pbmc6.markers <- FindAllMarkers(pbmc6, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
        pbmc1_1.markers <- FindAllMarkers(pbmc1_1, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
        pbmc2_1.markers <- FindAllMarkers(pbmc2_1, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
        pbmc3_1.markers <- FindAllMarkers(pbmc3_1, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
        pbmc4_1.markers <- FindAllMarkers(pbmc4_1, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
        pbmc5_1.markers <- FindAllMarkers(pbmc5_1, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
        pbmc6_1.markers <- FindAllMarkers(pbmc6_1, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
